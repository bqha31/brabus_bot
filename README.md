...
- Если клиент пишет в `Помощь`, бот отправляет запрос админам.
- Если клиент отправляет фото на `Оценка прически`, бот пересылает фото барберу.
- Барбер получает один текст с именем и номером клиента, затем два отдельных фото с подписями.
- Барбер отвечает **reply** на сообщение бота в WhatsApp — бот пересылает этот ответ клиенту прямо в чат.
...
После ответа мастера на фото-оценку бот может отправить клиенту: `Мы можем сделать такую стрижку ✅` и предложить записаться.

После отмены записи бот автоматически пишет клиенту: `Запись отменена. Хотите подобрать другое удобное время?`

Бот также отправляет follow-up напоминание через 3–4 недели после записи: `Пора обновить стрижку ✂️` → `Показать свободное время?`
